...
>
> 当前状态：[已完成] UCI Online Retail 一年版已下载并完成只读数据审计；尚未生成固定抽样数据、GT、机制输出或 MSE。
...
- [已确认] 正式数据采用 UCI Online Retail 一年版 `Online Retail.xlsx`；原始表是一行一个交易明细，须按本文件第 15 节规则过滤、按客户聚合后才成为用户购物篮。
...
| D10 | [已确认] 重复实验次数 | 固定 200 次 | 每个“机制 × \(\varepsilon\)”配置独立重复 | [冻结方案] 200 次 | 否 |
...
| D13 | [已确认] MSE 主口径 | 裁剪后为主、原始为补充 | 两者均保存并报告，预先指定主次 | [冻结方案] 裁剪后 MSE 为主，原始 MSE 为补充 | 否 |
...
| D10 | [已确认] 重复实验次数？ | 固定 200 次 | [冻结方案] 每配置独立重复 200 次 | 稳定性、运行量、统计汇总 |
...
| D13 | [已确认] MSE 用裁剪前还是裁剪后？ | 两者并报 | [冻结方案] 裁剪后为主、原始为补充 | 主结果、表图、结论 |
...
具体数据集属于实验自主设计内容，而不是缺失的硬性材料。

项目结构、数据读取接口和机制代码可以继续设计，但在确定正式数据前：
...
- B：公开购物篮数据集；
- C：自行生成的模拟交易数据；
- D：模拟数据用于验证，公开数据用于正式实验。

推荐优先确认老师是否另有数据；若没有，推荐选择D。

## 14. 已确认决策记录（2026-08-25）
...
- [已完成] 正式数据文件与格式已确认，并已完成只读结构与内容审计；结果见 `results/data_audit.json` 和 `results/data_audit.md`。
- [下一步门禁] 本阶段在数据审计处停止；固定单商品抽样、GT、Laplace、GRR、Basic UE 与 MSE 均须在后续阶段另行实现和验证。

## 15. 正式数据接入与冻结转换规则（2026-08-25）

### 15.1 数据来源与本地保存

- [已确认] 正式数据为 UCI Machine Learning Repository 的 **Online Retail** 一年版（数据集 ID 352，DOI `10.24432/C5BW33`，CC BY 4.0），覆盖 2010-12-01 至 2011-12-09 的全部交易，不按国家、月份或订单抽样。
- [已确认] 官方页面：<https://archive.ics.uci.edu/dataset/352/online%2Bretail>；官方压缩包：<https://archive.ics.uci.edu/static/public/352/online+retail.zip>。
- [已确认] 原始 `Online Retail.xlsx` 仅保存在 `data/raw/` 且不纳入 Git。文件大小为 23,715,344 bytes，SHA-256 为 `43465a06f2ccf7c8b5bd2892bc7defb52f97487934fe93b16ae4c3936424676d`。

### 15.2 明细纳入与购物篮构造

按以下规则判定交易明细；主排除原因严格按列出顺序唯一归类：

1. `CustomerID` 存在且可无损规范化为整数客户 ID；
2. `InvoiceNo` 去除首尾空白后不得以大小写不敏感的 `C` 开头；
3. `Quantity > 0`；
4. `UnitPrice > 0`；
5. `StockCode` 去除首尾空白并转大写后匹配 `^\d{5}[A-Z]{0,2}$`。

[已确认] 原始 Excel 的一行是一个交易明细，不是一名用户。过滤后按 `CustomerID` 聚合完整年度的全部保留明细，并在每名用户的篮内对规范化 `StockCode` 去重，才得到“一行一个用户、变长购物篮”的逻辑输入。服务/调整代码仅由上述正则规则排除，不使用人工黑名单或 `Description` 关键词。

### 15.3 审计结论与后续映射

- [已完成] 541,909 条原始明细全部对账；保留 396,337 条，聚合为 4,334 名用户和 3,659 个合格商品；38 名具有合法客户 ID 的用户在其他规则后形成空篮并被排除。
- [已完成] 用户内商品去重移除 130,116 条重复用户—商品明细；最终篮均非空、用户唯一且篮内无重复商品。
- [已确认] 后续商品映射仅按合格规范化 `StockCode` 的字典序稳定排序，并映射为连续内部 ID `1..d`，同时保留双向映射。当前审计阶段只预览规则，不物化映射。
- [范围限制] 本阶段不生成固定单商品数据、GT、机制输出或 MSE；合成 Excel 只用于审计程序单元测试。
